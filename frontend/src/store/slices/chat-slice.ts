...
export const markChatAsRead = createAsyncThunk(
  "chat/markChatAsRead",
  async (
    { chatRoomId, authToken }: { chatRoomId: number; authToken: string },
    { rejectWithValue },
  ) => {
    try {
...
const chatSlice = createSlice({
  ...
  reducers: {
    updateUnreadCount: (
      state,
      action: PayloadAction<{ chatRoomId: number; unreadCount: number }>,
    ) => {
      ...
      if (chatRoom) {
        ...
        state.totalUnreadCount = Math.max(
          0,
          state.totalUnreadCount + difference,
        );
      }
    },
    ...
    builder
      ...
      .addCase(markChatAsRead.fulfilled, (state, action) => {
        ...
        if (chatRoom) {
          ...
          state.totalUnreadCount = Math.max(
            0,
            state.totalUnreadCount - chatRoom.unreadCount,
          );
...
// Actions
export const { updateUnreadCount, addNewMessage, resetChatState } =
  chatSlice.actions;
