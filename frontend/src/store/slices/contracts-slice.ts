...
export const fetchContractById = createAsyncThunk(
  ...
  async (
    { contractId, authToken }: { contractId: number; authToken: string },
    { rejectWithValue },
  ) => {
    try {
      const response = await apis.getContractById({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<ApiResponse<Contract[]>>) => {
          state.loading.contracts = false;
          if (action.payload.status === "success" && action.payload.data) {
            state.contracts = action.payload.data;
          } else {
            state.error.contracts =
              action.payload.error || "Failed to fetch contracts";
          }
        },
      )
      ...
      .addCase(
        fetchContractById.fulfilled,
        (state, action: PayloadAction<ApiResponse<Contract>>) => {
          state.loading.contractDetails = false;
          if (action.payload.status === "success" && action.payload.data) {
            state.currentContract = action.payload.data;
          } else {
            state.error.contractDetails =
              action.payload.error || "Failed to fetch contract details";
          }
        },
      )
      ...
      .addCase(
        updateContractStatus.fulfilled,
        (state, action: PayloadAction<ApiResponse<Contract>>) => {
          ...
          if (action.payload.status === "success" && action.payload.data) {
            ...
            if (
              state.currentContract?.contractId === updatedContract.contractId
            ) {
              state.currentContract = updatedContract;
            }
          } else {
            state.error.updateStatus =
              action.payload.error || "Failed to update status";
          }
        },
      )
...
export const { clearCurrentContract, clearContractErrors } =
...
export const selectContracts = (state: RootState) => {
  ...
  return contracts.filter((contract) => {
    ...
    if (
      currentUser.role === "ROLE_FREELANCER" &&
...
export const selectContractsLoading = (state: RootState) =>
