...
export const ProfileField = ({
  ...
  const formatRatingValue = (val: string | number | undefined) => {
    if (variant === "rating" && val !== undefined && val !== null) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <label
          className={`text-xs font-medium ${config.label} uppercase tracking-wide`}
        >
          {label}
        </label>
      </div>
      ...
      <Card
        className={`border ${config.card} transition-all duration-200 hover:shadow-sm`}
      >
        <CardContent className="p-3">
          {isLoading ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <p
                className={`font-medium text-sm ${config.text} break-words line-clamp-2`}
              >
                {displayValue}
              </p>
            </div>
